...
class JsonStore:
    ...
    def resolve_support_request(self, request_id: str, response_text: str) -> dict[str, Any] | None:
        ...
        if found:
            ...
            has_pending = any(
                r.get("status") == "pending" and (r.get("groupId") == group_id or r.get("groupName") == group_name)
                for r in requests
            )
            coach = data.get("coach", {})
            for g in coach.get("groups", []):
    ...
    def check_github_submission(self, repo_url: str) -> dict[str, Any]:
        ...
        required_checks = [
            {"file": "README.md", "description": "Tài liệu giới thiệu sản phẩm & hướng dẫn chạy", "required": True},
            {"file": "docs/canvas.md", "description": "Canvas 7 dòng chốt bài toán & giá trị", "required": True},
            {"file": "docs/spec.md", "description": "Tài liệu đặc tả giải pháp SPEC", "required": True},
            {"file": "codebase/", "description": "Mã nguồn ứng dụng có thể chạy được", "required": True},
            {"file": "eval/", "description": "Bộ đánh giá Golden set (≥20 test cases)", "required": False},
        ]
        ...
        if not is_valid_format:
            return {
                ...
                "repo_url": repo_url,
                "status": "invalid_url",
                "summary": "Đường dẫn không hợp lệ. Vui lòng cung cấp link GitHub repository đúng định dạng (https://github.com/owner/repo).",
                "checks": [],
        ...
        result = {
            ...
            "repo_url": repo_url,
            "status": "verified",
            "summary": "AI Double Check thành công: Repository nộp bài có đầy đủ các tệp bàn giao bắt buộc (README, Canvas, Spec, Codebase). Đủ điều kiện nghiệm thu Lab.",
    ...
    def add_group_chat_message(self, message: dict[str, Any]) -> dict[str, Any]:
        ...
        for existing in messages[-10:]:
            if msg_id and existing.get("id") == msg_id:
                return existing
            if author and text and existing.get("author") == author and existing.get("text") == text:
                return existing

        if not msg_id:
            message["id"] = f"msg-{len(messages) + 1}-{int(datetime.now(timezone.utc).timestamp() * 1000)}"
